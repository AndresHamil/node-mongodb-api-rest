...
test("DELETE /sistema/accesos/usuarios/eliminarUsuario responde 401 sin token", async () => {
    const usuario = await ctx.createUser();

    const response = await request(app)
        .delete("/sistema/accesos/usuarios/eliminarUsuario")
        .send({
...
test("DELETE /sistema/accesos/usuarios/eliminarUsuario responde 200 cuando elimina un usuario", async () => {
...
test("DELETE /sistema/accesos/usuarios/eliminarUsuario responde 404 si el usuario no existe", async () => {
    const { token } = await ctx.createActorSession();
