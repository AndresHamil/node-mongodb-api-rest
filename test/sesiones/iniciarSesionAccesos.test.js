...
test("POST /sesiones/iniciarSesion devuelve contexto organizacional y accesos del perfil", async () => {
    ...
    const moduloGestion = await ctx.createModulo({
        nombre: "Gestion",
        codigo: "gestion",
        tipo: "gestion",
    ...
    const procesoGestion = await ctx.createProceso({
    ...
    const accesoGestionResponse = await request(app)
        ...
        .set("Authorization", `Bearer ${token}`)
        .send({
            procesoId: procesoGestion.insertedId.toString(),
            sucursales: [sucursal.insertedId.toString()],
            departamentos: [departamento.insertedId.toString()],
            perfiles: [perfil.insertedId.toString()],
    ...
    assert.equal(accesoGestionResponse.status, 200);
