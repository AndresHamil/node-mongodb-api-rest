...
test("PUT /gestion/usuarios/editarUsuario responde 401 sin token", async () => {
...
test("PUT /gestion/usuarios/editarUsuario responde 200 con edición válida", async () => {
...
test("PUT /gestion/usuarios/editarUsuario permite editar solo email enviando null en el resto", async () => {
    const { token } = await ctx.createActorSession();
...
test("PUT /gestion/usuarios/editarUsuario permite eliminar el telefono enviando string vacio", async () => {
    const { token } = await ctx.createActorSession();
...
test("PUT /gestion/usuarios/editarUsuario permite cambiar la contraseña con currentPassword y newPassword", async () => {
    const { token } = await ctx.createActorSession();
...
test("PUT /gestion/usuarios/editarUsuario elimina todas las sesiones del usuario cuando sesion=false", async () => {
    const { token } = await ctx.createActorSession();
...
test("PUT /gestion/usuarios/editarUsuario rechaza sesion=true porque el inicio de sesión solo ocurre desde login", async () => {
    const { token } = await ctx.createActorSession();
...
test("PUT /gestion/usuarios/editarUsuario rechaza el cambio si currentPassword es incorrecta", async () => {
    const { token } = await ctx.createActorSession();
    const usuario = await ctx.createUser({ password: "Temporal123!" });
...
test("PUT /gestion/usuarios/editarUsuario responde 422 con payload inválido", async () => {
    const { token } = await ctx.createActorSession();
    const usuario = await ctx.createUser();
...
test("PUT /gestion/usuarios/editarUsuario responde 404 si el usuario no existe", async () => {
    const { token } = await ctx.createActorSession();
...
test("PUT /gestion/usuarios/editarUsuario responde 409 cuando el email ya existe", async () => {
    const { token } = await ctx.createActorSession();
